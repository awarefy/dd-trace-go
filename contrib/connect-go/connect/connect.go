...
package connect // import "gopkg.in/DataDog/dd-trace-go.v1/contrib/gin-gonic/gin"
...
const componentName = "connect-go/connect"
...
func Middleware() connect.UnaryInterceptorFunc {
	interceptor := func(next connect.UnaryFunc) connect.UnaryFunc {
		return connect.UnaryFunc(func(
			...
			Logger.Info(
				...
				zap.String("Request Version", req.Header().Get("request_version")),
				zap.String("User Locale", req.Header().Get("locale")),
			)
			return next(ctx, req)
		})
	}
